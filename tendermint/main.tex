...
% These are the document margins
...
\usepackage{url}
\usepackage{tikz}
\usepackage{xcolor} % Use xcolor for modern color definitions
\usepackage{graphicx} % Use graphicx instead of graphics
\usepackage[section]{float} % Use [section] option for better float placement
\usepackage{verbatim}
\usepackage{listings}
\usepackage{alltt}
...
% --- OPTIMIZED COMMANDS ---
% We retain the custom look but use standard sectional commands for robustness.
...
\caption{Node Structure Example (To be replaced with Consensus Algorithm)}
\end{algorithm}
